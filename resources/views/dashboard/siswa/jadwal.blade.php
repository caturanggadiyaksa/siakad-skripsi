            ...
            <div class="container mt-2 mb-2">
                <div class="row">
                    ...
                    <div class="col-md-6 mt-4">
                        <div>
                            <h5> @foreach($m->jadwal as $j)
                                {{ $j->hari }}
                                @endforeach</h5>
                        </div>
                        <div class="card" style="height: 74px; border-radius: 20px 0px 0px 20px;">
                            <div class="d-flex">
                                <div class="" style="background: #4D44B5; height: 74px; width: 10%; border-radius: 20px 0px 0px 20px;"></div>
                                <div class="d-flex align-items-center" style="background: #C1BBEB; height: 74px; width: 90%;">
                                    <div class="d-flex mx-2 align-items-center">
                                        <div>
                                            <h6 style="color:#4D44B5;"> {{ $m->nama }}</h6>
                                        </div>
                                        <div class="row align-items-center mx-2">
                                            <div class="col">
                                                <p style="color: #111213;" class="m-0">@foreach($m->jadwal as $j)
                                                    {{ $j->tanggal }}
                                                    @endforeach</p>
                                                <span style="color: #121213;" class="m-0">
                                                    @foreach($m->jadwal as $j)
                                                    {{ $j->jam_mulai }}
                                                    @endforeach- @foreach($m->jadwal as $j)
                                                    {{ $j->jam_selesai }}
                                                    @endforeach
                                                </span>
                                            </div>
